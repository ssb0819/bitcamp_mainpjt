        ...
        success : function(data, status){
		   ...
		   var sessionId = "<%=((User)session.getAttribute("user")).getUserNo() %>" 
            ...
            $.each(data, function(key, value){
            	...
            	a = a + '<div class="commentArea" style="border-bottom:1px solid darkgray; margin-bottom: 15px;">'
            		+ '<div class="row">'
            		+ '<div class="col-md-10">'
            		+ '<div class="commentInfo'+value.replyNo+'">'
            		+ '<a href="/user/getProfile/'+value.userNo+'">'+value.nickname+'</a>' 
            		+ '</div>'
            		+ '<div class="commentContent'+value.replyNo+'"> '
            		+ '<p>'+value.replyContents+'</p>'
            		+ '</div>'
            		+ '</div>'
            		+ '<div class="col-md-2" style="text-align: right">'
            		+ '<a style="cursor:pointer; color: #FF914D;" onclick="commentDelete('+value.replyNo+');">삭제</a>'
            		+ ' <br>'+value.replyDate
            		+ '</div>'
            		+ '</div>'
            		+ '</div>';
