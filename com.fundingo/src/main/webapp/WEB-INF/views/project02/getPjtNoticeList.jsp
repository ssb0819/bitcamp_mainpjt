...
<%--		<div class="row">
			...
			<div class="col-md-6 text-left">
				<p class="text-default">전체 ${resultPage.totalCount } 건수
			</div>
		...
		<table class="table table-hover table-striped">
			<thead>
				<tr>
					...
					<th align="left" style="text-align: center;">제목</th>					
					<th align="left" style="text-align: center;">등록일</th>
				</tr>
			</thead>
			<tbody>
				...
				<c:forEach var="pjtNotice" items="${pjtNoticeList}">
					<tr>
						<td align="center" style="text-align: center;">${pjtNotice.pjtNoticeNo}</td>
						<td align="left" style="text-align: center; cursor: pointer;">${pjtNotice.pjtNoticeTitle}<input type="hidden"
							value="${pjtNotice.pjtNoticeNo}" /></td>
						<td align="left" style="text-align: center;"><fmt:formatDate value="${pjtNotice.pjtNoticeDate}" pattern="yyyy-MM-dd" /></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
	 	<div align="right">
	 		<c:if test="${sessionScope.user != null && sessionScope.user.userNo == project.user.userNo}">
